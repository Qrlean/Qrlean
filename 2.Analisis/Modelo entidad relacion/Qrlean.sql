...
INSERT INTO Tipo_asignaturas(nombre_asignatura) VALUES ('Ingles'),('Promover'),('Tecnica');
SELECT * from qrlean.Tipo_asignaturas;
...
('Kathie', 'Alvar', 3320320687,2,2, '0cc2ac8f98fd0177fae6f89097ada9f3', '73 Everett Court',2, 3883424358),
('Katuscha', 'Grishechkin', 8247825346,3,1, 'ebd48af1f270ef21f943579532a96fd1', '93699 Rockefeller Crossing',3, 3641176178),
('Romola', 'Rignoldes', 7577745441,4,2, '3a698de0ab79d99c1f23dc98030bfdcd', '619 Harbort Road',4, 3778849875),
('Adam', 'Mallinson', 9387752664,1,1, '3c217680c043d31e2a2721e0f0805aeb', '27 Valley Edge Trail',5, 3435608935),
('Caro', 'Itshak', 7398010323,2,2, 'd86f6ce759ebc50f76d28eafc3aca084', '9 Mendota Crossing',6, 3519855506),
('Correna', 'Bulpitt', 7687899597,3,1, '72b8ca963b02d9a2e4f39b2b775c5d16', '53 Algoma Crossing',7, 3322503446),
('Ethelind', 'Elington', 7800333589,4,2, '5980e0949699d60a6f86e478dd8d3c94', '74722 Forster Lane',8, 3915008778),
('Cameron', 'MacNockater', 3007853484,1,1, 'df861fe91302ceb317252a07f64470be', '67 Scofield Place',9, 3559894963),
('Lil', 'Kirwood', 3496939225,2,3, '72b1797ecc21c2319bda0d9053ce7f48', '5150 Randy Trail',10, 3690521840);
SELECT * FROM qrlean.Usuarios;

INSERT INTO Correos(email,id_usuario) VALUES ('bheinsen0@loc.gov',1),('avaughanhughes1@si.edu',2),('sandor2@soup.io',3),('ksunnex3@accuweather.com',4),('lsteggles4@prweb.com',5),('jcollingham5@behance.net',6),('mlegendre6@google.co.uk',7),('cpinsent7@myspace.com',8),('lduce8@auda.org.au',9),('ablaschke9@list-manage.com',10);
SELECT * FROM qrlean.correos;
...
INSERT INTO Asociacion_asignaturas(id_tipo_asignatura,id_ficha,id_instructor) VALUES (3,1,2),(3,2,4),(3,3,6),(3,4,8),(3,5,2);
SELECT * FROM qrlean.Asociacion_asignaturas;

INSERT INTO Clase(nombre_clase,dia,hora_inicio,hora_final,id_asignatura) VALUES ('Como programar','2021-05-05','12:05:05','12:05:05',1);
...
SELECT asociacion_asignaturas.id_asociacion_asignatura AS id ,tipo_asignaturas.nombre_asignatura,ficha.id_ficha,usuarios.nombres_usuario AS NombresInstructor, usuarios.apellidos_usuario AS ApellidosInstructor from qrlean.asociacion_asignaturas
	INNER JOIN qrlean.tipo_asignaturas ON tipo_asignaturas.id_tipo_asignatura = asociacion_asignaturas.id_tipo_asignatura
	INNER JOIN qrlean.ficha ON ficha.id_ficha = asociacion_asignaturas.id_ficha
    INNER JOIN qrlean.usuarios ON usuarios.id_usuario = asociacion_asignaturas.id_instructor
;
