        ...
        <>
            <Modal titulo="Eliminar persona" state={modal} setState={setModal}>
                <div className="flex flex-col text-justify w-full h-full overflow-x-hidden overflow-y-auto">
                    <h2 className="text-center text-3xl text-gray-800 font-semibold my-4">Advertencia.</h2>
                    <Warning className="w-32 h-32 mx-auto "/>
                    <p className="text-gray-800 text-base my-4 w-11/12 sm:w-3/5 mx-auto">Si desactivas el instructor este ya no tendrá acceso a su cuenta y automáticamente sera desasociado de todos los cursos-fichas en los cuales haya sido asociado, y todas las fechas de asistencia  que no hayan alcanzado a abordar serán desactivadas.</p>
                    <h2 className="text-center text-2xl text-red-500 font-semibold my-4">¿Esta segur@ de seguir?</h2>

                    <div className="flex-1 flex flex-row justify-center items-center">
                        <button className="bg-orange-500 text-gray-800 font-semibold text-1xl mx-4 p-4 rounded-lg outline-none " onClick={()=> setModal(false)}>No,cancelar operación.</button>
                        <button className="bg-orange-500 text-gray-800 font-semibold text-1xl mx-4 p-4 rounded-lg outline-none" onClick={()=> setModal(false)}>Si,realizar operación.</button>
                    </div>
                </div>
            </Modal>
            <motion.div layoutId="item-expandible" className="bg-white rounded-lg w-full lg:w-4/5 h-85 m-3 overflow-x-hidden max-w-full flex flex-col justify-center items-center" style={{boxShadow:"rgba(0, 0, 0, 0.09) 0px 2px 1px, rgba(0, 0, 0, 0.09) 0px 4px 2px, rgba(0, 0, 0, 0.09) 0px 8px 4px, rgba(0, 0, 0, 0.09) 0px 16px 8px, rgba(0, 0, 0, 0.09) 0px 32px 16px"}} >
                <div className='flex flex-row items-center h-full w-full  relative' >
                    ...
                    <motion.div layoutId="avatar" className=" hidden md:block" >
                        <Avatar size="150" className="m-5 shadow-lg" round={true} name={data.nombre} />
                    </motion.div>
                    <motion.div layoutId="contenido" className="flex flex-col justify-center items-start mx-2 sm:mx-16 h-full py-6 flex-1 w-full"  >
                        ...
                        <motion.h1 layoutId="titulo" className="capitalize text-base md:text-2xl text-left font-semibold text-gray-800">
                            Nombre: {data.nombre}
                        </motion.h1>
                        <motion.h2 layoutId="documento" className="text-base md:text-2xl text-left text-gray-800">
                    ...
                    <motion.div className="flex flex-col justify-center items-end mx-2 sm:mx-16 h-full py-6 " initial={{opacity:0}} animate={{opacity:1}} transition={{duration:0.5}}>
                        <svg viewBox="0 0 16 16" onClick={()=> router.push('/dashboard/admin/personas/editar/1234')} className="cursor-pointer fill-current h-8 w-8 sm:h-12 sm:w-12 my-8 text-gray-800 " xmlns="http://www.w3.org/2000/svg">
